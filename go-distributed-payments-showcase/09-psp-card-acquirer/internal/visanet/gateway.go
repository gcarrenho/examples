// Package visanet is the outbound adapter for Visa NET.
// It accepts authorization.CardTransaction and translates it to ISO 8583 internally.
// The service never sees ISO 8583 types — all protocol knowledge is encapsulated here.
package visanet
...
// Translates: authorization.CardTransaction → ISO 8583 0100 → ISO 8583 0110 → authorization.CardTransaction
func (g *Gateway) Authorize(ctx context.Context, txn authorization.CardTransaction) (authorization.CardTransaction, error) {
	...
	if err != nil {
		return authorization.CardTransaction{}, fmt.Errorf("visanet: marshal: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, g.endpoint+"/authorize", bytes.NewReader(body))
	if err != nil {
		return authorization.CardTransaction{}, fmt.Errorf("visanet: build request: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")

	resp, err := g.http.Do(req)
	if err != nil {
		return authorization.CardTransaction{}, fmt.Errorf("visanet: %w", err)
	}
	defer resp.Body.Close()

	var wire0110 wireResponse
	if err := json.NewDecoder(resp.Body).Decode(&wire0110); err != nil {
		return authorization.CardTransaction{}, fmt.Errorf("visanet: decode 0110: %w", err)
	}
	if wire0110.MTI != string(iso8583.AuthorizationResponse) {
		return authorization.CardTransaction{}, fmt.Errorf("visanet: unexpected MTI %q (want 0110)", wire0110.MTI)
	}
...
func mapResponseCode(rc iso8583.ResponseCode) error {
	...
	case iso8583.ResponseDuplicateSTAN:
		return authorization.ErrDuplicateSTAN
	default:
