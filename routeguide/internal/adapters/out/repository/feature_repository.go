...
func NewFeatureRepository(filePath string) (ports.FeatureRepository, error) {
	repo := &FeatureRepositoryImpl{}
	err := repo.loadFeatures(filePath)
	if err != nil {
		return nil, err
	}
	return repo, nil
...
func (repo *FeatureRepositoryImpl) GetFeature(ctx context.Context, point model.Point) (*model.Feature, error) {
	repo.mu.Lock()
	defer repo.mu.Unlock()
	for _, feature := range repo.savedFeatures {
		if feature.Location == point {
			return &feature, nil
		}
	}
	return &model.Feature{Location: point}, nil
}

func (repo *FeatureRepositoryImpl) ListFeatures(ctx context.Context, rect model.Rectangle) ([]model.Feature, error) {
	repo.mu.Lock()
	defer repo.mu.Unlock()
...
func inRange(point model.Point, rect model.Rectangle) bool {
